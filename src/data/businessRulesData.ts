...
export const DETAILED_BUSINESS_RULES: BusinessRuleSpec[] = [
  ...
  // 14. SERVICE-ZONE RULES
  {
    id: 'rule_14_service_zone_rules',
    ruleNumber: 14,
    category: 'LOGISTICS_ROUTING',
    title: 'Service-Zone Rules & Boundary Routing',
    summary: 'Geofenced operational territories across Nairobi defining intra-zone delivery SLAs, cross-zone surcharges, and hard exclusion perimeters.',
    businessObjective: 'Contains delivery radiuses within feasible boda boda travel envelopes to guarantee sub-35 minute delivery times.',
    keyFormulasOrParameters: [
      { label: 'Zone 1: Central (zone_nairobi_central)', value: 'Eastleigh, CBD, Pangani, Kariokor, Starehe (Radius ~4.5 km, SLA 15-25 mins)' },
      { label: 'Zone 2: East (zone_nairobi_east)', value: 'Kariobangi, Dandora, Umoja, Donholm, Buruburu, Kayole (Radius ~6.5 km, SLA 20-35 mins)' },
      { label: 'Zone 3: South (zone_nairobi_south)', value: 'Industrial Area, South B, South C, Imara Daima (FMCG Wholesale corridor)' },
      { label: 'Zone 4: West (zone_nairobi_west)', value: 'Kawangware, Kibera, Dagoretti, Kangemi, Westlands (Radius ~7.0 km, SLA 25-40 mins)' },
      { label: 'Intra-Zone Delivery Fee', value: 'Standard rates (KES 150 base)' },
      { label: 'Inter-Zone Transit Surcharge', value: '+KES 60 flat inter-zone bridge fee; distance capped at 9.0 km maximum' },
      { label: 'Absolute Boundary Cap', value: 'Orders exceeding 10.0 km road distance are strictly blocked at checkout' },
    ],
    stepByStepProtocol: [
      '1. Retailer GPS Registration: During shop onboarding, shop pin is mapped to primary Service Zone polygon.',
      '2. Wholesaler Zone Mapping: Depots are assigned to host zones.',
      '3. Checkout Geofence Evaluation:',
      '   - If Shop and Depot are in same zone: Classified as INTRA_ZONE (standard fee, 20-30m SLA).',
      '   - If Shop and Depot are in neighboring zones and distance ≤ 9km: Classified as INTER_ZONE (+KES 60, +15m SLA).',
      '   - If distance > 10km: Hard checkout block: "Delivery distance exceeds maximum boda coverage (10km). Please select an Eastlands depot."',
      '4. Dynamic Zone Rebalancing: Operations console can temporarily suspend a zone (e.g. during local civil unrest or severe flooding) without impacting other zones.',
    ],
    edgeCasesAndExceptions: [
      {
        scenario: 'Shop sits right on the boundary road between Central and East (e.g. Outer Ring Road)',
        resolution: 'Dual-Zone Affinity: Shop is granted dual-zone membership, accessing both Central and East wholesalers with zero inter-zone surcharge.',
      },
    ],
    kenyanMarketContext: 'Crossing from Kawangware (West) to Kariobangi (East) during Nairobi evening rush hour can take a boda over an hour due to gridlock on Uhuru Highway and Jogoo Road. Service zones keep riders operating in their familiar home turf.',
    slaOrThresholds: [
      { metric: 'Intra-Zone Delivery SLA', target: '< 35 minutes', penaltyOrAction: 'KES 50 credit if exceeded due to logistics delay' },
      { metric: 'Maximum Road Distance', target: '≤ 10.0 km', penaltyOrAction: 'Hard checkout block' },
    ],
